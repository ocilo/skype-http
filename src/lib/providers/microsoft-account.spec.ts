...
import {
  scrapLivePpftKey, getSkypeToken, GetSkypeTokenOptions, scrapLiveToken,
  scrapSkypeTokenResponse, SkypeTokenResponse
} from "./microsoft-account";
import {requestIo} from "../request-io";
import * as request from "request";
import {readTestResourceSync} from "../../test/test-resources";
...
describe("Microsoft Account provider", function(this: Mocha.IContextDefinition) {
  ...
  describe.skip("getSkypeToken", async function() {
  ...
  describe("scrapLivePpftKey", function() {
    ...
    for (const item of items) {
      it(`file: ${JSON.stringify(item.file)}`, function() {
        const html = readTestResourceSync(item.file);
        const actualPpftKey = scrapLivePpftKey(html);
  ...
  describe("scrapLiveToken", function() {
    ...
    for (const item of items) {
      it(`file: ${JSON.stringify(item.file)}`, function() {
        const html = readTestResourceSync(item.file);
        const actualLiveToken = scrapLiveToken(html);
